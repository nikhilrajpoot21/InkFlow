...
export default function Home() {
  // Fake data for demo – replace later with API data
  const trendingPosts = [
  ...
  <main className="flex-1 p-10 overflow-y-auto">
    ...
    <section className="mb-12">
      <h2 className="text-2xl font-bold text-[#2c2c2c] mb-6">🔥 Trending Posts</h2>
    ...
    <section>
      ...
      <div className="space-y-6">
        {allPosts.map((post) => (
          ...
          >
            <h3 className="text-lg font-semibold text-[#2c2c2c]">{post.title}</h3>
            <p className="text-[#6b6b6b] mt-2">{post.preview}</p>
            <div className="flex justify-between items-center mt-4 text-sm">
              <span className="text-[#bfae94]">
                {post.time} • By {post.author}
              </span>
              <Link
                to={`/post/${post.id}`}
                className="text-[#2c2c2c] hover:text-[#bfae94]"
